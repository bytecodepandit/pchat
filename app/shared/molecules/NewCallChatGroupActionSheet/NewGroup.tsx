...
const NewGroup = ({ close, onScroll }: NewGroupProps) => {
  const { userWithSection, userForGroupCreation } = useSelector(
    (state: Store) => state,
  );
  const selectUserRef = useRef<any>(null);
  const dispatch = useDispatch();

  const toggleUserFromParcipants = (
    item: ChatItem,
    isSelected: boolean,
    index: number,
  ) => {
    ...
    setTimeout(() => {
      selectUserRef.current.scrollToOffset({
        offset: userForGroupCreation.data.length * 40,
        animated: true,
      });
    });
  };
      ...
      <Box
        ...
        <View style={{ flex: 1, justifyContent: 'center' }}>
          <Text
            ...
            lineHeight={18}
            padding={0}>
            {i18n.t('add')} {i18n.t('participants')}
          </Text>
          <Text textAlign="center" color="primary" lineHeight={16} padding={0}>
            {userForGroupCreation.data.length}/ 286
          </Text>
        </View>
        <TouchableOpacity
          onPress={close}
          style={{ justifyContent: 'flex-end' }}
          disabled={userForGroupCreation.data.length === 0}>
          <Text
            textAlign="right"
            style={{
              color:
                userForGroupCreation.data.length === 0
                  ? colors.disabled
                  : colors.darkBlue,
            }}>
            {i18n.t('next')}
  ...
  const _renderUserItem = (item: ChatItem) => {
    ...
    return (
      <TouchableWithoutFeedback
        onPress={() => toggleUserFromParcipants(item, isSelected, index)}>
        <UserInlineCard
          name={item.title}
          content={
  ...
  const _renderSelectedUsers = ({
    ...
    <TouchableWithoutFeedback
      onPress={() => toggleUserFromParcipants(item, true, index)}>
      <Box
        style={{
        ...
        <AntDesign
          style={{ position: 'absolute', top: verticalScale(0), right: 0 }}
          name="closecircle"
          color={colors.primary}
          size={RFValue(20)}
        />
        ...
        <View style={{ zIndex: 5 }}>
          <AvatarAtom source={{ uri: item.image }} />
        </View>
        <Text
          style={{
            maxWidth: scale(50),
