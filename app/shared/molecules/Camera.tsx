...
const Camera = (
  { close, capture, selectPhoto }: CameraScreenProps,
  ref: any,
) => {
  ...
  const _renderHeader = () => {
    return (
      <Box
        ...
        <Box>
          <TouchableWithoutFeedback
            onPress={() => setIsFlashMode(!isFlashMode)}>
            <Ionicons
              name={isFlashMode ? 'flash-off' : 'flash'}
              color={colors.white}
              size={RFValue(24)}
            />
          </TouchableWithoutFeedback>
  ...
  const _renderFooter = () => {
    return (
      <Box
        ...
        <TouchableOpacity
          onPress={() =>
            launchImageLibrary({ quality: 0.5, mediaType: 'photo' }, (event) =>
              selectPhoto(event),
            )
          }>
        ...
        <View
          ...
          <Text color="white" marginTop="s" fontSize={RFValue(12)}>
            Hold for video, tap for photo
          </Text>
        </View>
        <TouchableOpacity onPress={() => setIsBackCamera(!isBackCamera)}>
          <Ionicons
            name={isBackCamera ? 'camera-reverse-outline' : 'camera-outline'}
            color={colors.white}
            size={RFValue(24)}
          />
        </TouchableOpacity>
  ...
  const _renderSharedImageVideos = () => {
    return localStoredImages.length > 0 ? (
      <FlatList
        horizontal={true}
        data={localStoredImages}
        showsHorizontalScrollIndicator={false}
        renderItem={({ item, index }) => (
          <Box paddingRight="hxs" key={`thum_image_${index}`}>
            <Image
              style={{
  ...
  return (
    <Overlay isVisible={showCamera} overlayStyle={{ padding: 0, margin: 0 }}>
      <RNCamera
        ref={cameraRef}
        captureAudio={false}
        zoom={zoom}
          ...
          <View style={{ overflow: 'hidden', position: 'relative' }}>
            <Animated.View
              ...
              <GestureRecognizer
                ...
                <AnimatedIcon
                  name={showThumnails ? 'minus' : 'chevron-thin-up'}
                  color={colors.white}
                  style={{ height: verticalScale(40) }}
                  size={RFValue(showThumnails ? 50 : 30)}
                />
              </GestureRecognizer>
