...
import {
  FETCH_CALL_HISTORY_REJECTED,
  FETCH_CALL_HISTORY_RESOLVED,
...
export function* fetchUsersCallHistory({ payload }: Params) {
  ...
  try {
    ...
    const res: any = yield restApiService.invoke(
      ApiEndPoints.getUsersCallHistoryByUserId,
      { userId },
      { ...query },
    );
    if (res.status === 200) {
      ...
      yield put({
        ...
        payload:
          query.callingType === 'ALL'
            ? result
            : result.filter(
              (elem: CallHistoryItem) =>
                elem.callingType === query.callingType,
            ),
      });
    }
  } catch (error) {
