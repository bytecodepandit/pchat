...
import { FETCH_STATUS_REJECTED, FETCH_STATUS_RESOLVED } from "@app/store/actions/action-types";
...
export function* fetchStatus(params: Params) {
    ...
    try {
        // @ts-ignore
        const res: any = yield restApiService.invoke(ApiEndPoints.getUsersStatusByUserId, { userId });
        if (res.status === 200) {
            yield put({
