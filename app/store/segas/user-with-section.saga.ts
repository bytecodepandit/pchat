...
import { FETCH_USER_WITH_SECTIONS_REJECTED, FETCH_USER_WITH_SECTIONS_RESOLVED } from "../actions/action-types";
...
const getState = (state: Store) => state;
 ...
 function* fetchUserWithSection({ payload }: Params): any {
    ...
    try {
        const res: any = yield restApiService.invoke(ApiEndPoints.getUserWithSectionByUserId, { userId }, null);
        if (res.status === 200) {
            yield put({
