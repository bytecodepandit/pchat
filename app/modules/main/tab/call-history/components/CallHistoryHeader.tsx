...
import {
  getUsersCallHistory,
  setCallingType,
} from '@app/store/actions/call-history.action';
...
const CallHistoryHeader = ({ editList, addCall }: CallHistoryHeaderProps) => {
  ...
  const getCallHistory = (callingType: CallingType) => {
    ...
    dispath(
      getUsersCallHistory({ userId: 1, scroll: false, query: { callingType } }),
    );
  };
  ...
  return (
    <Header
      leftComponent={
        <Pressable
          onPress={() => {
            editList(!isEditingMode);
            setIsEditingMode(!isEditingMode);
          }}
          style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
          <Text color="darkBlue">
            {!isEditingMode ? i18n.t('edit') : i18n.t('done')}
          </Text>
        </Pressable>
      }
